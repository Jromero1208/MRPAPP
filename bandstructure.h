...
namespace rpa {
	...
	public:
		...
		// constructor for specified tbfile
		bandstructure(const rpa::parameters<Field,MatrixTemplate,ConcurrencyType>& parameters,
			          ConcurrencyType& concurrency, const kDomain& kmeshIn,
			          bool caching, std::string file):
			param(parameters),
			conc(concurrency),
			nbands(param.nOrb),
			kmesh_(param,conc,0,2), // not being used
		...
		// Constructor without kmesh input for emptybands;
		bandstructure(const rpa::parameters<Field,MatrixTemplate,ConcurrencyType>& parameters,
								ConcurrencyType& concurrency, int x):
			param(parameters),
			conc(concurrency),
			nbands(param.nOrb),
			kmesh_(param,conc,0,2,1),
			kmesh(kmesh_),
			caching_(false),
			cachedK(0),
			ev(0),
		...
		void getEkAndAk(VectorType& kvec, VectorType& eigenvals, ComplexMatrixType& eigenvects, int spin=1)  {
			...
			//	std::cerr << "eigenvector before :" << eigenvects << std::endl;
				phaseFactor(kvec,eigenvects);
			//	std::cerr << "eigenvector after :" << eigenvects << std::endl;
				return;
			} else {
		...
		void readCSVFile(std::string file) {
